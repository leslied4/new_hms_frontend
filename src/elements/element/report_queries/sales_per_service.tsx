...
<div class="container-fluid card p-3">
    <div class="d-flex justify-content-between align-items-center px-3">
        <div>
            <h4 class="font-weight-bold">Sales Per Service</h4>
    ...
    <div class="">
        ...
        <!-- php: = $this->Form->create(null, ['type' => 'get', 'id' => 'patient_base_filter_medicine_availability_salesperservice']); -->
        <div class="row mr-2" style="float: right;">
            <div class="d-flex align-items-center">
                <label style="white-space:nowrap" class="mt-1">Date Range:&nbsp;</label>
                <SearchableSelectField class="form-control" style="width:100%;" name="dateSelelectFields" id="patient_base_dates_base_medicine_salesperservice">
            ...
            <div class="d-flex align-items-center mr-2">
                ...
                <SearchableSelectField class="form-control" style="width:100%;" name="category" id="patient_base_category_salesperservice">
            ...
            <div class="d-flex align-items-center mr-2">
                ...
                <input class="form-control" style="width:100%;" value="" name="search" id="search_salesperservice" placeholder="search" />
            </div>
    ...
    <div class="container-fluid bg-white py-3 px-0 mt-4" style="border-radius: 5px;">
            ...
            <table id="salesperservice_analysis_table" class="table mt-3">
                <thead>
                    <th className='' id="columnTitle">Service</th>
                ...
                <thead id="summary_head">
                    <th className='' id="">Summary</th>
                    <th className='px-0 text-center' id="summary_gross"><div className='mt-1 pt-1' >0 <span>0</span></div></th>
                    <th className='px-0 text-center' id="summary_discount"><div className='mt-1 pt-1 d-flex justify-content-between px-2' >0 <span>0</span></div></th>
                    <th className='px-0 text-center' id="summary_tax"><div className='mt-1 pt-1 d-flex justify-content-between px-2' >0 <span>0</span></div></th>
                    <th className='px-0 text-center' id="summary_refunds"><div className='mt-1 pt-1 d-flex justify-content-between px-2' >0 <span>0</span></div></th>
                    <th className='px-0 text-center' id="summary_net"><div className='mt-1 pt-1 d-flex justify-content-between px-2' >0 <span>0</span></div></th>
                </thead>
                <tbody id="salesperservice_analysis_table_data">
                </tbody>
            </table>
...
<script>
    ...
    $("#patient_base_filter_medicine_availability_salesperservice").submit(function (e) {
        e.preventDefault()
        try {
            let res = {}
            let url = ""
            let date_itself = document.getElementById('patient_base_dates_base_medicine_salesperservice').value != '' ? 
                                calculateDateRange(document.getElementById('patient_base_dates_base_medicine_salesperservice').value)['current'] : null
            if (date_itself) {
            ...
            if (document.getElementById('search_salesperservice').value != '') {
                url = (url == '' ? url : (url+'&')) + serialize({
                    'search': document.getElementById('search_salesperservice').value,
                })   
                res = {
                    ...res,
                    'search': document.getElementById('search_salesperservice').value,
            ...
            if (document.getElementById('patient_base_category_salesperservice').value != '') {
                url = (url == '' ? url : (url+'&')) + serialize({
                    'category': document.getElementById('patient_base_category_salesperservice').value,
                })   
                res = {
                    ...res,
                    'category': document.getElementById('patient_base_category_salesperservice').value,
                }
                let val = $('#patient_base_category_salesperservice option:selected').text()
                $('#columnTitle').text(val)
            } else {

                $('#columnTitle').text("Service")

            }
            requestsalesperserviceAnalysis(url)
        } catch (error) {
            console.log(error)
        }
        return false
    ...
    function requestsalesperserviceAnalysis(url) {
        table = $('#salesperservice_analysis_table').DataTable();
        table.destroy();
        $.ajax({
			type: 'GET',
			url: "<!-- php: echo $this->Url->build( ['controller' => 'ReportQueries', 'action' => 'salesByProduct'] ); -->?"+url,
			data: {},
			beforeSend: function(){
			...
			success: function (res){
                ...
                let result = processsalesperserviceTable(res)
                table = $('#salesperservice_analysis_table').DataTable();
                table.destroy();
                $('#salesperservice_analysis_table_data').html(result)
                table = $('#salesperservice_analysis_table').DataTable();
			}
		});
    };

    function processsalesperserviceAnalysis(all_items) {
        let total_q = 0
        let total_q_p = 0
        let fortyfive_q = 0
        let fortyfive_q_p = 0
    ...
    function processsalesperserviceTable(data) {
        let result = ''
    ...
    //     // requestsalesperserviceAnalysis()
    // })
...
export default function ElementElementReportQueriesSalesPerService() {
