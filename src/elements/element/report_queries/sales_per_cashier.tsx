...
<div class="container-fluid card p-3">
    <div class="d-flex justify-content-between align-items-center px-3">
        <div>
            <h4 class="font-weight-bold">Sales By Cashier</h4>
    ...
    <div class="">
        ...
        <!-- php: = $this->Form->create(null, ['type' => 'get', 'id' => 'patient_base_filter_medicine_availability_salespercashier']); -->
        <div class="row mr-2" style="float: right;">
            <div class="d-flex align-items-center">
                <label style="white-space:nowrap" class="mt-1">Date Range:&nbsp;</label>
                <SearchableSelectField class="form-control" style="width:100%;" name="dateSelelectFields" id="patient_base_dates_base_medicine_salespercashier">
            ...
            <!-- <div class="d-flex align-items-center mr-2">
                ...
                <SearchableSelectField class="form-control" style="width:100%;" name="category" id="patient_base_category_salespercashier">
            ...
            <div class="d-flex align-items-center mr-2">
                ...
                <input class="form-control" style="width:100%;" value="" name="search" id="search_salespercashier" placeholder="search" />
            </div>
    ...
    <div class="container-fluid bg-white py-3 px-0 mt-4" style="border-radius: 5px;">
            ...
            <table id="salespercashier_analysis_table" class="table mt-3">
                <thead>
                    <th className='' id="clinicianColumnTitle">User</th>
                    <th className='px-0 text-center'>Gross</th>
                    <th className='px-0 text-center'>Discount</th>
                    <th className='px-0 text-center'>Tax</th>
                    <th className='px-0 text-center'>Refund</th>
                    <th className='px-0 text-center'>Net</th>
                </thead>
                <thead id="clinician_summary_head">
                    <th className='' id="">Summary</th>
                    <th className='px-0 text-center' id="" style="font-weight: bold">0</th>
                    <th className='px-0 text-center' id="" style="font-weight: bold">0</th>
                    <th className='px-0 text-center' id="" style="font-weight: bold">0</th>
                    <th className='px-0 text-center' id="" style="font-weight: bold">0</th>
                    <th className='px-0 text-center' id="" style="font-weight: bold">0</th>
                </thead>
                <tbody id="salespercashier_analysis_table_data">
                </tbody>
            </table>
...
function calculateDateRange(selectedOption) {
    ...
    $("#patient_base_filter_medicine_availability_salespercashier").submit(function (e) {
        e.preventDefault()
        try {
            let res = {}
            let url = ""
            let date_itself = document.getElementById('patient_base_dates_base_medicine_salespercashier').value != '' ? 
                                calculateDateRange(document.getElementById('patient_base_dates_base_medicine_salespercashier').value)['current'] : null
            if (date_itself) {
            ...
            if (document.getElementById('search_salespercashier').value != '') {
                url = (url == '' ? url : (url+'&')) + serialize({
                    'search': document.getElementById('search_salespercashier').value,
                })   
                res = {
                    ...res,
                    'search': document.getElementById('search_salespercashier').value,
            ...
            // if (document.getElementById('patient_base_category_salespercashier').value != '') {
            //     url = (url == '' ? url : (url+'&')) + serialize({
            //         'category': document.getElementById('patient_base_category_salespercashier').value,
            //     })   
            //     res = {
            //         ...res,
            //         'category': document.getElementById('patient_base_category_salespercashier').value,
            //     }
            //     let val = $('#patient_base_category_salespercashier option:selected').text()
            ...
            // }
            requestsalespercashierAnalysis(url)
        } catch (error) {
            console.log(error)
        }
        return false
    ...
    function requestsalespercashierAnalysis(url) {
        table = $('#salespercashier_analysis_table').DataTable();
        table.destroy();
        $.ajax({
			type: 'GET',
			url: "<!-- php: echo $this->Url->build( ['controller' => 'ReportQueries', 'action' => 'salesByCashier'] ); -->?"+url,
			data: {},
			beforeSend: function(){
			...
			success: function (res){
                ...
                let result = processsalespercashierTable(res)
                table = $('#salespercashier_analysis_table').DataTable();
                table.destroy();
                $('#salespercashier_analysis_table_data').html(result)
                table = $('#salespercashier_analysis_table').DataTable();
			}
		});
    };

    function processsalespercashierAnalysis(all_items) {
        let total_q = 0
        let total_q_p = 0
        let fortyfive_q = 0
        let fortyfive_q_p = 0
    ...
    function processsalespercashierTable(data) {
        let result = ''
                ...
                <tr>
                    ...
                    <td className='p-1'>
                        <div className="d-flex justify-content-between">
                            ...
                            <p>\${item.gross - item.returns - item.tax - item.discount}</p>
                        </div>
                    </td>
                </tr>
           \`
        ...
        summary = \`
            ...
            <th className='p-1'>
                <div className="d-flex justify-content-between">
                    ...
                    <p style="font-weight: bold">\${res.gross - res.returns - res.discount - res.tax}</p>
                </div>
            </th>
    ...
    //     // requestsalespercashierAnalysis()
    // })
...
export default function ElementElementReportQueriesSalesPerCashier() {
