...
<div class="card  card-box">
    <div class="card-head">
        <header>Account Adjustments Reconciliation</header>
    </div>
    <div class="card-body ">
        <div class="row">
                ...
                <div class="borderBox-body">
                    <div class="tab-content">
                        ...
                        <div class="table-responsive">
                            <table id="account-adjustments-reconciliations-table" class="table">
                                <thead>
                                    <tr>
                                        <th>Date Added</th>
                                        <th>Source</th>
                                ...
                                <tbody>
                                   ...
                                   <tr>
                                        ...
                                        <td>Abukari Einus</td>
                                        <td>
                                            <button data-toggle="modal" data-target="#report_moneyin_recon" class="btn btn-xs btn-primary">Report Money In</button>
                                        </td>
                                   </tr>
                                   <tr>
                                        <td>04/12/2023 05:52:57</td>
                                        ...
                                        <td>Abukari Einus</td>
                                        <td>
                                            <button data-toggle="modal" data-target="#report_moneyout_recon" class="btn btn-xs btn-danger">Report Money Out</button>
                                        </td>
                                   </tr>
                                </tbody>
                            </table>
...
<div class="modal fade" id="report_moneyin_recon" tabindex="-1" aria-hidden="true">
...
<div class="modal fade" id="report_moneyout_recon" tabindex="-1" aria-hidden="true">
   <div class="modal-dialog modal-md  modal-dialog-centered" role="document">
      <div class="modal-content">
         <div class="container px-0 border border-2 border-<!-- php: = $theme2 -->">
            <div class="container-fluid pr-0 bg-<!-- php: = $theme2 -->">
               <div class="d-flex align-items-center justify-content-between">
                  <h4 class="text-slate-900 my-0">Report Money Out</h4>
                  <div>
                     <button data-dismiss="modal" aria-label="Close" class="btn custom-btn border-0 bg-white btn-sm px-1 mr-1 d-flex align-items-center justify-content-center"><i class="fa fa-times text-primary"></i> </button>
                  </div>
               </div>
               ...
               <div class="row mt-4">
                     ...
                     <SearchableSelectField class="form-control input-height selectpicker show-menu-arrow show-tick" data-size="10" title="Select cash category" name="category_id[]" id="category_id" data-live-search="true"  required>
                            ...
                            <!-- php: foreach($Accounts as $account) { -->
                                ...
                                <option data-content="<!-- php: = $account->account_name =="" ? $account->banks_list->name : $account->account_name --> <span class='<!-- php: if($account->accounts_tags[0]->attribute == "MONEY OUT") { echo 'badge bg-danger'; } else if($account->accounts_tags[0]->attribute == "MONEY IN OUT") { echo 'badge bg-secondary'; } -->'><!-- php: = $account->transaction_code --></span>" value="<!-- php: = $account->id -->"><!-- php: = $account->account_name =="" ? $account->bank_name : $account->account_name -->
                                </option>
                            <!-- php: } -->
                            <!-- php: } -->
...
<script>
    $('#account-adjustments-reconciliations-table').DataTable();
...
export default function ElementElementAccountsAccountAdjustmentsReconciliation() {
