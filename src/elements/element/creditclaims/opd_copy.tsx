								...
								<div class="card-body">
								  <div class="row">
									<div class="col-md-4">
										<div class="input-group" style="">
											...
											<SearchableSelectField class="form-control selectpicker show-menu-arrow show-tick ml-1" data-size="10" id="status_filter_opd" title="Status Filter"  data-live-search="true" width="12px"required>
									...
									<div class="col-md-8">
									  <!-- php: = $this->Form->create(null, ['type' => 'get']); -->
										<div class="input-group" style="max-width: 600px; float: right;">
											<button type="button" class="btn btn-warning mr-1">Waiting</button>
											<button type="button" class="btn btn-primary mr-1">Assigned</button>
								  ...
								  <div class="table-scrollable">
									<table class="table table-hover order-column full-width table-checkable">
										...
										<tbody>
										<!-- php: $i = 0; -->
										<!-- php: foreach ($opdInvoices as $invoice): $diff=date_diff($invoice->date_added,$now); $days = trim($diff->format("%R%a"), "+"); -->
											<!-- php: if($days >= 20){ $color = "danger"; $status = 'Overdue'; }elseif($days >= 10){ $color = "#FFA07A"; }elseif($days < 10){ $color = "#98FB98"; $status = 'New'; } -->
											<!-- php: $i++; -->
											<tr class ="main" id="<!-- php: = $status -->">
												<td><!-- php: = $i --></td>
												<td>
												...
												<td>	
													<a href="<!-- php: =$this->Url->build(['controller'=>'CreditClaims','action'=>'viewClaimDetails', $invoice->id]) -->" class="btn <!-- php: = Cake\Core\Configure::read('Classes.View') --> btn-xs patient_view_patient_info">View</a>
													<button type="button" class="btn <!-- php: = Cake\Core\Configure::read('Classes.View') --> btn-xs patient_view_patient_inf" data-toggle="modal" data-target="#details_<!-- php: = $invoice->id -->">Details</button>						
												</td>
												<tr>
													<td colspan="12" id="more_details">
											...
											<div class="modal fade" id="details_<!-- php: = $invoice->id -->" tabindex="-1" role="dialog" aria-labelledby="select" aria-hidden="true">
										...
										<!-- php: endforeach; -->
										<tfoot>
											<tr class ="main" style="background-color: #eaeef3;">
												<td></td>
												<td></td>
												<td></td>
												<td></td>
												...
												<td></td>
												<td><b>Total</b></td>
												<td><!-- php: = $sumOfOpdInvoiceFinalAmounts --></td>
											</tr>
										</tfoot>
										</tbody>
									</table>
...
// 		// $('td:not(#more_details)').stop().slideToggle();

// 		// $target.closest("tr").next().find("td[colspan=12]").slideToggle();
...
// $('#more_details').click(false);


// $("#status_filter_opd").change(function(){
...
export default function ElementElementCreditclaimsOpdCopy() {
