					...
					<div class="row">
						<div class="col-md-12">
							<div class="card card-topline-<!-- php: = $theme1 -->">
								<div class="card-head">
                                    <header>Submissions - Company</header>
									<div class="tools">
										<!-- <a class="fa fa-repeat btn-color box-refresh" id="reset" href="javascript:;"></a>
										<a class="t-collapse btn-color fa fa-chevron-down" href="javascript:;"></a>
										<a class="t-close btn-color fa fa-times" href="javascript:;"></a> -->
										<span>Export:</span>
										<a target="_blank" class="btn-color btn-outline btn btn-outline red" title="Export To Excel" href="<!-- php: = Cake\Routing\Router::url(['controller' => 'CreditClaims', 'action' => 'exportDataToExcel', 3]) -->"> Excel <!-- php: = $this->Html->image('../assets/img/excel.png', ['style' => 'width: 25px; height: 25px']) --></a>

										<a target="_blank" class="btn-color btn-outline btn btn-outline red" title="Export To PDF" href="<!-- php: = Cake\Routing\Router::url(['controller' => 'CreditClaims', 'action' => 'exportDataToPdf', 3]) -->"> PDF <!-- php: = $this->Html->image('../assets/img/pdf.png', ['style' => 'width: 25px; height: 25px']) --></a>
									</div>
								</div>
								<div class="card-body">
								  <div class="row">
									...
									<table class="table table-hover order-column full-width table-checkable">
										...
										<!-- php: foreach ($company as $invoice): -->
											...
											<tr>
												...
												<td><!-- php: = $invoice->patient_visit->patient->patient_age --> years <span class='badge badge-info'><!-- php: = $invoice->patient_visit->patient->folder_number --></span></td>
												<td><!-- php: = $invoice->date_added->nice() --></td>
												<td class="">
													<span class="badge badge-pill badge-primary">
														<!-- php: = isset($invoice->patient_visit->patient_visit_purpose->name) ? $invoice->patient_visit->patient_visit_purpose->name : "" -->
													</span>
													<span class="badge badge-pill badge-secondary">
														<!-- php: = isset($invoice->patient_visit->specialty->name) ? $invoice->patient_visit->specialty->name : "" -->
													</span>
												</td>
												<td>
													<!-- php: if(count($invoice->patient_visit->patient_visit_admissions)): -->
													...
													<!-- php: endif; -->
													<!-- php: = $invoice->co_payment == 1 ? "<span class='badge badge-primary'>COPAY: Enabled</span>" : "<span class='badge badge-primary'>COPAY: DISABLED</span>" -->
														
													<!-- php: // isset($invoice->patient_visit->patient_visit_admissions[0]->bed->ward->name) ? $invoice->patient_visit->patient_visit_admissions[0]->bed->ward->name : "" --> <!-- php: // isset($invoice->patient_visit->patient_visit_admissions[0]->bed) ? $invoice->patient_visit->patient_visit_admissions[0]->bed->name : "" -->
												</td>
												<td>
													<span class='badge badge-primary'><!-- php: = $invoice->insurance_profile_policy->name --></span>
													<span class='badge badge-secondary'><!-- php: = $invoice->insurance_profile_policy->insurance_profile->name --></span>
												...
												<td>	
													<!-- <a target="_blank" href="<!-- php: // Cake\Routing\Router::url(['controller' => 'CreditClaims', 'action' => 'claimPdf', $invoice->id, 'claim_pdf', '_ext' => 'pdf']) -->" class="btn btn-xs btn-outline" escape=false>
                                                        PDF
                                                    </a>	 -->
													<a target="_blank" href="<!-- php: = Cake\Routing\Router::url(['controller'=>'Billings','action'=>'insuranceBillingReport',$invoice->id, 'insurance']) -->" class="btn btn-xs btn-outline" type="button"> <span><i class="fa fa-print"></i> PDF</span> </a>				
												</td>
											</tr>
										<!-- php: endforeach; -->
										</tbody>
...
export default function ElementElementCreditclaimsSubmittedCompany() {
