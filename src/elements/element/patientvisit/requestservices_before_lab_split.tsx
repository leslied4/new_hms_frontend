		...
		<div class="borderBox-body">
			<div class="tab-content">
				<div class="tab-pane request_lab_content" id="borderBox_request_lab">
						...
						<table class="table table-hover order-column full-width" id="labs_table">
							...
							<tbody>
								...
								<tr>
									...
									<td><!-- php: = $requestLabItem->has('invoice_item') ? "<span class='badge badge-" . ($requestLabItem->invoice_item->status_id == 27 ? "success" : "danger") . "'>" . ($requestLabItem->invoice_item->status_id == 27 ? "&nbsp;Paid&nbsp;" : "Pend") . "</span... --></td>
					...
					<div style="margin-top: 40px;" id="previous_labs_button">
					    <div style="float: right;"><a class="btn btn-sm btn-success" onclick="javascript:$('#previous_labs').toggle(500); moveToId('previous_labs_button');">View Previous Labs</a></div>
						<div style="clear: both;"></div>
					</div>

					<div id="previous_labs" style="display: none;">
						
						<!-- php: foreach($patientVisits as $pVisit) { $dateDiff = $pVisit->date_created->diffForHumans($selectedVisit->date_created); if(sizeof($pVisit->request_labs) <= 0 || $pVisit->id == $selectedVisit->id || (!stristr($dateDiff, 'ago') && !stristr($date... -->
							...
							<div class="table-scrollable">
								<table class="table table-hover order-column full-width" id="labs_table">
									...
									<tbody>
										<!-- php: foreach ($pVisit->request_labs as $requestLabItem) { -->
											<tr>
												<td><!-- php: = $requestLabItem->has('investigation') ? $requestLabItem->investigation->name : '' --></td>
												<td><!-- php: = $requestLabItem->has('lab_test') ? $requestLabItem->lab_test->name : '' --></td>
												<td><!-- php: = h($requestLabItem->date_created) --></td>
				...
				<div class="tab-pane request_medication_content" id="borderBox_request_medication">
					...
					<div style="margin-top: 40px;" id="previous_medications_button">
					    <div style="float: right;"></div>
						<div style="clear: both;"></div>
					</div>

					<div id="previous_medications" style="display: none;">
						
						<!-- php: foreach($patientVisits as $pVisit) { $dateDiff = $pVisit->date_created->diffForHumans($selectedVisit->date_created); if(sizeof($pVisit->request_medications) <= 0 || $pVisit->id == $selectedVisit->id || (!stristr($dateDiff, 'ago') && !strist... -->
							...
							<div class="table-scrollable">
								<table class="table table-hover order-column full-width" id="medications_table">
									...
									<tbody>
										<!-- php: foreach ($pVisit->request_medications as $requestMedicationItem): -->
											<tr>
												<td><!-- php: = h($requestMedicationItem->date_created) --></td>
												<td><!-- php: = $requestMedicationItem->external_medication == 0 ? ($requestMedicationItem->has('drug_stock') ? $requestMedicationItem->drug_stock->drug->full_name : '') : ($requestMedicationItem->has('drug') ? $requestMedicationItem->drug->full_name : '... --></td>
												<td><!-- php: = $requestMedicationItem->has('dosage_form') ? $requestMedicationItem->dosage_form->name : '' --></td>
												<td><!-- php: = $requestMedicationItem->frequency --></td>
												<td><!-- php: = $requestMedicationItem->number_of_days --></td>
												<td><!-- php: = $requestMedicationItem->instructions --></td>
											</tr>
										<!-- php: endforeach; -->
									</tbody>
								</table>
						...
						<div class="row">
									...
									<div class="col-md-7">
										<SearchableSelectField class="form-control input-height selectpicker show-menu-arrow show-tick" onchange="$('#medicationForm').validate().element('#drug_stock_id');" data-size="10" title="Drug / Form / Dosage" name="drug_stock_id" id="drug_stock_id" data-live-search="true" required >
											...
											<!-- php: foreach($drugStocks as $drugStock) { -->
													<option value="<!-- php: = $drugStock->id -->" data-content="<span style='margin-bottom: 5px;' class='badge badge-<!-- php: = in_array($drugStock->stock_basket_id, [1, null]) ? 'warning' : 'secondary' -->'><i class='fa fa-<!-- php: = in_array($drugStock->stock_basket_id, [1, null]) ? 'building' : 'shopping-basket' -->'></i></span> <!-- php: = h($drugStock->drug->full_name) --><br/><span style='magin-bottom: 5px;'><span style='font-weight: 700;'> Price:</span> <span>GHS<!-- php: = $this->Number->precision($drugStock->unit_cost_price, 2) --></span>&nbsp;&nbsp;&nbsp; <span style='font-weight: 700;'>Qty: </span><span><!-- php: = $drugStock->quantity_left --></span><br/><span class='badge badge-primary '>Batch: <!-- php: = $drugStock->batch_number --></span> <span class='badge badge-danger '>Expiry: <!-- php: = $drugStock->expiry_date->format('Y-m-d') --></span><hr style='margin-top: 10px; margin-bottom: 0px;'/>"><!-- php: = h($drugStock->drug->full_name) --></option>
												<!-- php: } -->
										</SearchableSelectField>
									</div>
								</div>
							...
							<div class="col-md-6">
								...
								<div class="form-group row">
									...
									<div class="col-md-7">
										<!-- php: // default to patient if no provider is found $billTo = $selectedVisit->has('insurance_profile_policy_id') ? $selectedVisit->patient_insurance_profile_policy_id->insurance_profile_policy_id : -1; -->
					...
					<div class="form-body">
						<div class="form-group row">
							...
							<div class="form-group row">
								...
								<div class="col-md-5">
									<!-- php: // default to patient if no provider is found $billTo = $selectedVisit->has('patient_provider') ? $selectedVisit->patient_insurance_profile_policy_id->insurance_profile_policy_id : -1; -->
									<SearchableSelectField class="form-control input-height selectpicker show-menu-arrow show-tick" data-size="5" data-live-search="true" class="form-control input-height" name="bill_to_id" id="bill_to_id" required >
										<option value="">Select Bill To</option>
										<option <!-- php: = $billTo == -1 ? 'selected' : '' --> value="-1">Patient</option>
										<!-- php: foreach($providers as $provider) { -->
							...
							<tbody>
								<!-- php: foreach ($requestServices as $service): -->
									<tr class="odd gradeX">
										...
										<td><!-- php: = $service->has('invoice_item') ? "<span class='badge badge-" . ($service->invoice_item->status_id == 27 ? "success" : "danger") . "'>" . ($service->invoice_item->status_id == 27 ? "&nbsp;Paid&nbsp;" : "Pend") . "</span> " . $this->Number->... --></td>
					...
					<!-- php: = $this->Form->create(null, ['url' => ['controller' => 'RequestServices', 'action' => 'addRequestService', $patient->id, $selectedVisit->id] , 'id'=>'serviceForm']); -->
						...
						<div class="form-body">
							...
							<div class="form-group row">
								...
								<div class="col-md-5">
									<!-- php: // default to patient if no provider is found $billTo = $selectedVisit->has('patient_insurance_profile_policy_id') ? $selectedVisit->patient_insurance_profile_policy_id->insurance_profile_policy_id : -1; -->
									<SearchableSelectField class="form-control input-height selectpicker show-menu-arrow show-tick" data-size="5" data-live-search="true" class="form-control input-height" name="bill_to_id" id="bill_to_id" required >
										<option value="">Select Bill To</option>
										<option <!-- php: = $billTo == -1 ? 'selected' : '' --> value="-1">Patient</option>
										<!-- php: foreach($providers as $provider) { -->
...
export default function ElementElementPatientvisitRequestservicesBeforeLabSplit() {
