...
type RequestRadiologyIndexResponse = {
  summary?: {
    total?: number;
    regular?: number;
    walkin?: number;
...
const normalizeType = (value: string): TypeFilter => {
  ...
  if (key === 'walkin' || key === 'walkinscan' || key === 'walkin_scan') return 'walkin';
  return 'all';
};
...
export default function RequestRadiologiesIndex() {
  ...
  const [data, setData] = useState<RequestRadiologyIndexResponse | null>(null);
  ...
  }, [type, searchValue, statusId, priorityId, doctorId, specialtyId, fromDate, toDate, page, setSearchParams]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (fromDate) params.set('from_date', fromDate);
        if (toDate) params.set('to_date', toDate);
        const response = await api.get<RequestRadiologyIndexResponse>(`/legacy/request-radiologies/index/?${params.toString()}`);
        if (!mounted) return;
        setData(response);
      } catch (err) {
        if (!mounted) return;
        setError(err instanceof Error ? err.message : 'Unable to load requested scans.');
      } finally {
    ...
    load().catch(() => {
      if (!mounted) return;
      setError('Unable to load requested scans.');
      setIsLoading(false);
    });
    return () => {
      mounted = false;
    };
  }, [type, searchValue, statusId, priorityId, doctorId, specialtyId, fromDate, toDate, page]);
  ...
  const title = useMemo(() => {
    if (type === 'walkin') return 'Requested Scans - Walk-In';
    if (type === 'regular') return 'Requested Scans - Regular';
          ...
          <table className="min-w-full text-xs">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-3 py-2 text-left font-semibold">Scans</th>
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={9} className="px-3 py-8 text-center text-slate-500">Loading requested scans...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={9} className="px-3 py-8 text-center text-slate-500">No requested scans found.</td></tr>
              ) : (
                rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-2 text-slate-700">{asText(row.scan_summary) || 'N/A'} {row.request_count ? `(${row.request_count})` : ''}</td>
                      ...
                      <td className="px-3 py-2">
                        <Link to={`/RequestRadiologies/view_request?visit_id=${encodeURIComponent(row.visit_id)}`} className="rounded border border-slate-300 px-2 py-1 text-[11px] font-semibold text-slate-700">
                          View
                        </Link>
                      </td>
                    </tr>
