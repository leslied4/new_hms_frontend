...
const sourcePath = 'templates/NurseStation/view_patient_nurse_symptoms.php';
const rawHtml = `
<style>
...
<h3 class="card-header">
    ...
    <!-- php: = $patient->full_name --> <span class="badge badge-primary"><!-- php: = $patient->folder_number --></span> Symptoms
</h3>
...
<script>
    ...
    Object.values(grouped).forEach((ele) => {
        ...
        $('#view-all-odqs').append(\`
            <div class="category-header home-medications" onclick="toggleCategory('\${ele.patient_visit_id}')">
                <span class="icon-chevron expanded" id="home-icon"></span>
                <span class="icon-home"></span>
                <span>\${ele.patient_visit_date}</span>
            </div>
            
            <div class="test-list" id="\${ele.patient_visit_id}-list">
    ...
    function viewAllSymptomsPatientData(odqData, id) {
		...
		const patientData = {
			symptoms: odqData.reduce((acc, item) => {
				...
				if (item.odq_id in acc) {
					acc[item.odq_id].entries.push(leRes)
					
				} else {
					acc[item.odq_id] =  {
						id: item.odq_id, // unique key
		...
		Object.values(patientData.symptoms).forEach(symptom => {
			...
			node.innerHTML = \`
				<div class="node-header symptom \${symptom.entries.map(entry => \`odqView-\${entry.role}\`).join(' ')}" data-node="\${nodeId}">
					<svg class="chevron expanded" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function NurseStationViewPatientNurseSymptomsPage() {
  return (
    <PageShell title="NurseStation/view_patient_nurse_symptoms.php" sourcePath={sourcePath}>
