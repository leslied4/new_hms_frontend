...
const sourcePath = 'templates/Notifications/index.php';
const rawHtml = `
<!--main body-->
<div class="row">
	<div class="col-md-12">
					...
					<table class="table table-hover table-striped order-column full-width">
						...
						<!-- php: // $i = 0; foreach ($notifications as $notification): // $i++; -->
							<tr class="odd gradeX">
								...
								<td><!-- php: = $notification->notification_type->name --></td>
								<td>
								...
								<td>
									<a <!-- php: = $notification->notification_type_id == 5 ? 'target="_blank"' : '' --> title="View" href="<!-- php: =$this->Url->build(['controller'=>'Notifications', 'action'=>'processNotification', $notification->id]) -->" class="btn btn-xs btn-info">
										view
									</a>	
									<a title="Delete Notification" href="<!-- php: =$this->Url->build(['controller'=>'Notifications', 'action'=>'delete', $notification->id]) -->" class="btn btn-xs btn-danger">
										Delete
									</a>	
								</td>
							</tr>
...
<script>
	$(document).ready(function($){
		var myresultMain = $.parseJSON('<!-- php: echo json_encode([]) -->');
...
export default function NotificationsIndexPage() {
  return (
    <PageShell title="Notifications/index.php" sourcePath={sourcePath}>
